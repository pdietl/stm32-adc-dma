...
#define DT_DRV_COMPAT st_stm32_adc_dma
...
static const uint32_t table_triggers[] = {
...
static void adc_stm32_start_conversion(const struct device *dev)
{
	...
	if (ret != 0) {
		LOG_ERR("ADC ERR: RX DMA config failed!");
		return; // -EINVAL;
	}
...
void adc_stm32_dma_rx_cb(const struct device *dma_dev, void *user_data,
...
static int adc_stm32_init(const struct device *dev)
{
	...
	data->dma_rx.blk_cfg.source_address = LL_ADC_DMA_GetRegAddr(adc, LL_ADC_DMA_REG_REGULAR_DATA);
...
#define ADC_DMA_CHANNEL_INIT(index, dir, dir_cap, src_dev, dest_dev)	\
	...
	.dma_cfg = {							\
		...
		.dest_burst_length = 1,					\
		.block_count = 1,					\
		.dma_callback = adc_stm32_dma_##dir##_cb,		\
	},								\
...
.dma_##dir = {				\
	COND_CODE_1(DT_INST_DMAS_HAS_NAME(index, dir),			\
		 (ADC_DMA_CHANNEL_INIT(index, dir, DIR, src, dest)),	\
		 (NULL))				\
	}
...
static struct adc_stm32_data adc_stm32_data_##index = {			\
	...
	ADC_DMA_CHANNEL(index, rx, RX, PERIPHERAL, MEMORY),		\
